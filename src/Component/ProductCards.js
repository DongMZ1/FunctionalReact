      ...
      {loginState.productcart.map((product) => (
        <Row className={`white-bg blue-color user-select-none rounder-border mb-2 ${innerWidth > 768 && 'mx-3'} lightblue-border`}>
          ...
          <Col xs={7}>
            ...
            <div className={`${innerWidth < 768? 'font-12p':'font-14p'}`}>
            Price Per Item : <b>{product.price}</b> <span className='ml-2'>
              {innerWidth < 768 && <br />}
              Subtotal: <b>{(product.price * product.number).toFixed(2)}</b></span>
            </div>
